...
namespace Monotouch.TableView.UrlImageStore
{
	...
	{
		void UrlImageUpdated(string id, UIImage image, int row, int section);
	}
	...
	public class UrlImageStore : NSObject
	{
		...
		public UIImage RequestImage(string id, string url, IUrlImageUpdated notify, int row, int section)
		{
			...
			factory.StartNew (delegate {				
				try
				{
					...
					notify.UrlImageUpdated(id, img, row, section);
				}
				catch (Exception ex)
				{
